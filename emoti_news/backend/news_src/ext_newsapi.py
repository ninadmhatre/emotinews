...
Country = str
Category = str
APIResponse: TypeAlias = dict[Country, dict[Category, dict[str, Any]]]


class NewsApiOrgProvider(API):
    api_name = "newsapi.org"

    def __init__(self, api_key: str | None = None):
        self._api_key = api_key or SOURCE_API_KEY.get(self.api_name)
        self.client = NewsApiClient(api_key=self._api_key)
    ...
    def _get_top_headlines(self, country: str, category: str) -> dict[str, Any]:
        return self.client.get_top_headlines(category=category, country=country)

    def get_headlines(self, countries: list[str], categories: list[str]) -> APIResponse:
        headlines: APIResponse = {}
                    ...
                    status.meta.update(
                        {
                            ...
                            "error": error,
                            "rows_fetched": len(top_headlines["articles"]),
                        }
                    )
    ...
    def parse(self, response: APIResponse, keep_last_n_days: int = 1) -> list[Article]:
